...
const InputEnter = ({content, setContent, viewingPasteId}) => {
  ...
  return (
    <div className='mt-7 sm:mt-12 flex flex-wrap sm:justify-center gap-y-4 gap-4 w-[94%]'>
      <input disabled={ viewingPasteId && true } type="text" value={ viewingPaste?.title || title } onChange={(e) => setTitle(e.target.value)} placeholder='Enter Title Here....' className='bg-[#0F0F0F] border border-[rgb(29,40,58)] rounded-[6px] w-full h-12 outline-none text-white px-3 sm:w-[545px] text-xl' />
      {
        viewingPasteId ? null : 
